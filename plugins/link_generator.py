...
async def batch(client: Client, message: Message):
    while True:
        ...
        if f_msg_id:
            ...
            await first_message.reply("❌Erreur\n\nce message ne provient pas de mon canal DB.", quote = True)
            continue

    while True:
        try:
            second_message = await client.ask(text = "Transfert moi le dernier message de la liste (Avec la balise)..\n\nou envoie moi son lien", chat_id = message.from_user.id, filters=(filters.forwarded | (filters.text & ~filters.forwarded)), timeout=60)
        except:
            return
    ...
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("🔁 Partage le lien", url=f'https://telegram.me/share/url?url={link}')]])
...
async def link_generator(client: Client, message: Message):
    ...
    link = f"https://t.me/{client.username}?start={base64_string}"
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("🔁 Partagr le lien", url=f'https://telegram.me/share/url?url={link}')]])
